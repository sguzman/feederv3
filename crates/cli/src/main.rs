...
use feedrv3_core::infra::config::{ConfigLoader, LoadedConfig};

#[derive(Parser)]
#[command(author, version, about = "feedrv3 ops CLI")]
struct Args {
    #[command(subcommand)]
    command: Command,
}
...
async fn main() -> Result<(), String> {
    ...
    match args.command {
        ...
        Command::Clean {
            ...
            let cfg_path = pick_config_path(config_path);
            let LoadedConfig { app, .. } = ConfigLoader::load(&cfg_path)
                .await
                .map_err(|e| e.to_string())?;
            ...
            if let Err(e) = std::fs::remove_dir_all(&app.log_file_directory) {
                if e.kind() != std::io::ErrorKind::NotFound {
