...
use crate::models::{EntryListResponse, EntrySummary, SearchQuery};
...
pub async fn search_entries(
    ...
    let q = query.q.trim();
    if q.is_empty() {
        return Err(ServerError::new(StatusCode::BAD_REQUEST, "q required"));
    }
    ...
    if let Some(pool) = &state.postgres {
        ...
        builder.push(" OR fi.summary ILIKE ");
        builder.push_bind(&pattern);
        builder.push(" OR fi.description ILIKE ");
        builder.push_bind(&pattern);
        builder.push(")");
    ...
    builder.push_bind(user_id);
    builder.push(" WHERE (");
    builder.push("lower(fi.title) LIKE lower(");
    builder.push_bind(&pattern);
    builder.push(") OR lower(fi.summary) LIKE lower(");
    builder.push_bind(&pattern);
    builder.push(") OR lower(fi.description) LIKE lower(");
    builder.push_bind(&pattern);
    builder.push(")");
