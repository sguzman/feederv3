...
impl SqliteRepo {
    ...
    async fn ensure_feed_base_poll_column(
        pool: &SqlitePool,
    ...
    async fn ensure_feed_state_note_column(pool: &SqlitePool) -> Result<(), String> {
        let has_column: Option<i64> = sqlx::query_scalar(
        ...
        I::IntoIter: Send,
    {
        ...
        while let Some(feed) = iter.next() {
            ...
            if chunk.len() == chunk_size {
                self.upsert_chunk(&chunk, zone).await?;
                total += chunk.len();
                chunk.clear();
            }
        }
    ...
    async fn upsert_chunk(&self, feeds: &[FeedConfig], zone: &Tz) -> Result<(), String> {
    ...
    async fn set_synchronous(pool: &SqlitePool, mode: &str) -> Result<String, String> {
        ...
        Ok(Self::synchronous_to_string(prev))
    }

    fn synchronous_to_string(val: i64) -> String {
        match val {
            0 => "OFF",
            1 => "NORMAL",
            2 => "FULL",
...
impl Repo for SqliteRepo {
    async fn migrate(&self, _zone: &Tz, default_poll_seconds: u64) -> Result<(), String> {
      ...
      CREATE TABLE IF NOT EXISTS feed_items(
        ...
        Self::ensure_feed_base_poll_column(&self.pool, default_poll_seconds).await?;
        Self::ensure_feed_state_note_column(&self.pool).await?;
    ...
    where
        ...
        I::IntoIter: Send,
    {
        let prev_sync = Self::set_synchronous(&self.pool, "NORMAL").await?;
        let res = self.do_upsert_chunks(feeds, chunk_size.max(1), zone).await;
        let _ = Self::set_synchronous(&self.pool, &prev_sync).await;
        res
    }
      ...
      "#,
        ...
        let feeds = rows
            ...
            .map(|r| FeedConfig {
                ...
                url: r.url,
                domain: r.domain,
                base_poll_seconds: r.base_poll_seconds.max(0) as u64,
            })
            .collect::<Vec<_>>();
