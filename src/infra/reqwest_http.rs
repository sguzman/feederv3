...
impl ReqwestHttp {
  ...
  fn status_error_kind(code: StatusCode) -> Option<ErrorKind> {
    let n = code.as_u16();
    if (400..500).contains(&n) { Some(ErrorKind::Http4xx) }
    else if (500..600).contains(&n) { Some(ErrorKind::Http5xx) }
    else { None }
  }

  fn parse_last_modified(headers: &header::HeaderMap) -> Option<i64> {
    let v = headers.get(header::LAST_MODIFIED)?.to_str().ok()?;
    DateTime::parse_from_rfc2822(v).ok().map(|dt| dt.with_timezone(&Utc).timestamp_millis())
  }

  fn parse_etag(headers: &header::HeaderMap) -> Option<String> {
    headers.get(header::ETAG).and_then(|v| v.to_str().ok()).map(|s| s.to_string())
  }
}
  ...
  async fn head(&self, url: &str) -> HeadResult {
    ...
    match self.client.head(url).send().await {
      Ok(resp) => {
        ...
        let error = status.and_then(|s| Self::status_error_kind(StatusCode::from_u16(s).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR)));
        HeadResult { status, etag, last_modified, error, latency_ms }
      }
      Err(e) => {
        warn!(url, error = %e, "HTTP HEAD failed");
        let latency_ms = start.elapsed().as_millis() as u64;
        HeadResult { status: None, etag: None, last_modified: None, error: Some(Self::classify_error(&e)), latency_ms }
      }
    }
  }
  ...
  async fn get(&self, url: &str) -> GetResult {
    ...
    match self.client.get(url).send().await {
      Ok(resp) => {
        ...
        let latency_ms = start.elapsed().as_millis() as u64;
        let error = status.and_then(|s| Self::status_error_kind(StatusCode::from_u16(s).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR)));
        GetResult { status, body, etag, last_modified, error, latency_ms }
      }
      Err(e) => {
        warn!(url, error = %e, "HTTP GET failed");
        let latency_ms = start.elapsed().as_millis() as u64;
        GetResult { status: None, body: None, etag: None, last_modified: None, error: Some(Self::classify_error(&e)), latency_ms }
      }
    }
  }
}
